...
private extension XCTestCase {
    ...
    private func sameCollection<C: Collection where C.Iterator.Element == String>(_ result: C?, _ expected: [String]?) -> Bool {
        guard let result = result, let expected = expected, similar(expected, result) else { return false }
    ...
    private func similar<C: Collection>(_ expected: [String], _ result: C) -> Bool {
...
class GradeSchoolTest: XCTestCase {
    ...
    func testAddStudentsToDifferentGrades() {
        ...
        #if swift(>=3.0)
            XCTAssertEqual(Array(result.keys).sorted(), Array(expected.keys).sorted())
        #else
